...
你会根据 Apple Watch / HealthKit 数据和用户自定义 persona 生成中文健康报告。
...
5. 建议必须贴合用户 persona；如果 persona 为空，则按普通久坐办公/学习场景处理。
...
def normalize_payload(payload: dict) -> dict:
    if "analysis_date" in payload:
        ...
        payload.setdefault("persona", "久坐办公或学习人群；希望获得温和、具体、可执行的健康建议。")
        return payload
    ...
    return {
        ...
        "persona": payload.get("persona", "久坐办公或学习人群；希望获得温和、具体、可执行的健康建议。"),
...
def call_openai(payload: dict, metrics: dict) -> dict:
    ...
    user_prompt = {
        ...
        "personalization_contract": {
            "identity": payload.get("persona", "久坐办公或学习人群"),
            "pressure_context": "如果用户 persona 提到压力、学习、工作、照护、运动目标或恢复需求，请优先贴合这些背景。",
